...
import com.example.controller.ValidationError
...
interface ExceptionInterface {
    ...
    @ApiResponses(value = [
        ApiResponse(
            responseCode = "500", description = "Internal server error", content = [Content(
                mediaType = mediaType, schema = Schema(implementation = ExceptionModel::class)
            )]
        )
    ])
    ...
    @ApiResponses(value = [
        ApiResponse(
            responseCode = "422", description = "Input validation failed", content = [Content(
                mediaType = mediaType, schema = Schema(implementation = ValidationExceptionModel::class)
            )]
        )
    ])
    ...
    @ApiResponses(value = [
        ApiResponse(
            responseCode = "400", description = "Input parsing failed", content = [Content(
                mediaType = mediaType, schema = Schema(implementation = ExceptionModel::class)
            )]
        )
    ])
